package generator;

import com.google.common.base.Objects;
import generator.AbstractWebsite;
import org.eclipse.xtend2.lib.StringConcatenation;

@SuppressWarnings("all")
public class Download extends AbstractWebsite {
  public String path() {
    return "download.html";
  }
  
  public String getLink(final String platform) {
    String _fileExtension = this.fileExtension(platform);
    return (("distro/eclipse-jee6-generator-kepler-" + platform) + _fileExtension);
  }
  
  public String fileExtension(final String platform) {
    String _xifexpression = null;
    boolean _or = false;
    boolean _equals = Objects.equal(platform, Download.WIN_32);
    if (_equals) {
      _or = true;
    } else {
      boolean _equals_1 = Objects.equal(platform, Download.WIN_64);
      _or = _equals_1;
    }
    if (_or) {
      _xifexpression = ".zip";
    } else {
      _xifexpression = ".tar.gz";
    }
    return _xifexpression;
  }
  
  public final static String OSX_64 = "macosx-cocoa-x86_64";
  
  public final static String OSX_32 = "macosx-cocoa";
  
  public final static String LINUX_64 = "linux-gtk-x86_64";
  
  public final static String LINUX_32 = "linux-gtk";
  
  public final static String WIN_64 = "win32-x86_64";
  
  public final static String WIN_32 = "win32";
  
  public CharSequence contents() {
    StringConcatenation _builder = new StringConcatenation();
    CharSequence _headline = this.headline("Download");
    _builder.append(_headline, "");
    _builder.newLineIfNotEmpty();
    _builder.append("<div id=\"page\">");
    _builder.newLine();
    _builder.append("\t");
    _builder.append("<div class=\"inner\">");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("<div class=\"container clearfix\">");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<h2>Downloads</h2>");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<hr>");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<!--table one starts-->");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<div class=\"span4\">");
    _builder.newLine();
    _builder.append("\t\t\t\t");
    _builder.append("<div class=\"pricing\">");
    _builder.newLine();
    _builder.append("\t\t\t\t    ");
    _builder.append("<table>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<tfoot>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t\t");
    _builder.append("<td></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("</tfoot>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<tbody>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t");
    _builder.append("<td class=\"focus\">Update Sites <span>Nutze den Eclipse Update Manager</span></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("<td style=\"text-align: center;\">");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("<a");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("rel=\"popover\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("data-content=\"This is an Eclipse update site URL, which you need to paste into the update manager within Eclipse!\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("data-original-title=\"Right-click and \'Copy Link\'\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("href=\"http://jee6-generator.itemis.de/updates/kepler/\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("class=\"has-popover btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("style=\"width: 70%;\">Kepler</a></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t");
    _builder.append("<td style=\"text-align: center;\">");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t");
    _builder.append("<a");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t  ");
    _builder.append("rel=\"popover\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("data-content=\"This is an Eclipse update site URL, which you need to paste into the update manager within Eclipse!\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("data-original-title=\"Right-click and \'Copy Link\'\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t  ");
    _builder.append("href=\"http://jee6-generator.itemis.de/updates/snapshot/\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("class=\"has-popover btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    \t");
    _builder.append("style=\"width: 70%;\">Snapshot</a></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t");
    _builder.append("</tbody>");
    _builder.newLine();
    _builder.append("\t\t\t\t    ");
    _builder.append("</table>");
    _builder.newLine();
    _builder.append("\t\t\t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<div class=\"span4\">");
    _builder.newLine();
    _builder.append("\t\t\t\t");
    _builder.append("<div class=\"pricing\">");
    _builder.newLine();
    _builder.append("\t\t\t\t    ");
    _builder.append("<table>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<tfoot>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t\t");
    _builder.append("<td></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("</tfoot>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<tbody>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t\t\t");
    _builder.append("<td class=\"focus\">Full Eclipse <span>Einfach runterladen und entpacken</span></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<td style=\"text-align: center; white-space: nowrap;\">");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<a href=\"");
    String _link = this.getLink(Download.OSX_64);
    _builder.append(_link, "\t\t\t\t\t\t    \t");
    _builder.append("\"");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("class=\"btn btn-primary btn-large\" style=\"width: 45%;\">OS X 64 Bit</a>&nbsp;");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<a href=\"");
    String _link_1 = this.getLink(Download.OSX_32);
    _builder.append(_link_1, "\t\t\t\t\t\t    \t");
    _builder.append("\"");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("class=\"btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("style=\"width:20%;\">32 Bit</a></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<td style=\"text-align: center; white-space: nowrap;\">");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<a href=\"");
    String _link_2 = this.getLink(Download.LINUX_64);
    _builder.append(_link_2, "\t\t\t\t\t\t    \t");
    _builder.append("\"");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("class=\"btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("style=\"width: 45%;\">Linux 64 Bit</a>&nbsp;");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<a href=\"");
    String _link_3 = this.getLink(Download.LINUX_32);
    _builder.append(_link_3, "\t\t\t\t\t\t    \t");
    _builder.append("\"");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("class=\"btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("style=\"width:20%;\">32 Bit</a></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("<tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<td style=\"text-align: center; white-space: nowrap;\">");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<a href=\"");
    String _link_4 = this.getLink(Download.WIN_64);
    _builder.append(_link_4, "\t\t\t\t\t\t    \t");
    _builder.append("\"");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("class=\"btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("style=\"width: 45%;\">Windows 64 Bit</a>&nbsp;");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("<a href=\"");
    String _link_5 = this.getLink(Download.WIN_32);
    _builder.append(_link_5, "\t\t\t\t\t\t    \t");
    _builder.append("\"");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("class=\"btn btn-primary btn-large\"");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    \t");
    _builder.append("style=\"width:20%;\">32 Bit</a></td>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("</tr>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("</tbody>");
    _builder.newLine();
    _builder.append("\t\t\t\t    ");
    _builder.append("</table>");
    _builder.newLine();
    _builder.append("\t\t\t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<div class=\"container\">");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<div class=\"container\">");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("<h2>Installation Instructions</h2>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("<hr>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t  ");
    _builder.append("<div class=\"span9\">");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<p>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("<strong>  The JEE6 Generator is implemented in Java, so you must have a");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<a href=\"http://www.oracle.com/technetwork/java/index.html\">Java");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("Runtime Environment</a> installed in order to proceed.");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("</strong>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</p>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<p>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("There are two easy ways to get the JEE6 Generator up and running. A pre-configured Eclipse distribution is available");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("which has already all the necessary plug-ins installed. Alternatively, you can install the JEE6 Generator into your");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("existing Eclipse by means of the Eclipse update mechanism.");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</p>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<!--  section -->");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<section id=\"InstallDistro\" style=\"padding-top: 68px; margin-top: -68px;\">");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<h2>  Install Pre-Configured Eclipse");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</h2>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("<ol>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<li>Download the distribution from above that matches your OS.</li>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<li>Unzip the archive into the directory of your choice.");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<strong>  Windows Users should choose a directory close to the root since the zip contains a");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("deeply nested folder structure. The maximum path length on windows may not exceed 256 characters.");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("</strong>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("</li>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<li>Launch Eclipse and select the workspace location. A workspace location is the directory for your user data");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("and project files.");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("</li>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("</ol>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</section>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<!--  section -->");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<section style=\"padding-top: 68px; margin-top: -68px;\">");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<h2>  Install the JEE6 Generator from Update Site");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</h2>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("<p>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("If you have an Eclipse running :");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</p>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("<ul>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<li>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("Choose");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<strong>  Help -&gt; Install New Software...");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("</strong> from the menu bar and");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("<strong>  Add...");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t");
    _builder.append("</strong> Insert one of the update site URLs from above.");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("This site aggregates all the necessary and optional components and dependencies of the JEE6 Generator.");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("</li>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<li>Select the <i>JEE6 DSL</i> from the category <i>JEE6-Generator</i> and");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("complete the wizard by clicking the <i>Next</i> button until you can click <i>Finish</i>.");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("</li>");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("<li>");
    _builder.newLine();
    _builder.append("\t\t\t\t\t\t    ");
    _builder.append("After a quick download and a restart of Eclipse, the JEE6 Generator is ready to use.");
    _builder.newLine();
    _builder.append("\t\t        ");
    _builder.append("</li>");
    _builder.newLine();
    _builder.append("\t\t      ");
    _builder.append("</ul>");
    _builder.newLine();
    _builder.append("\t\t    ");
    _builder.append("</section>");
    _builder.newLine();
    _builder.append("\t\t  ");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("<div class=\"container\">");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<h2>FAQs</h2>");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<hr />");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<div class=\"span9\" id=\"faq\">");
    _builder.newLine();
    _builder.append("\t\t\t\t");
    _builder.append("<div class=\"accordion\" id=\"accordion2\">");
    _builder.newLine();
    _builder.append("\t\t\t\t    ");
    StringConcatenation _builder_1 = new StringConcatenation();
    _builder_1.append("Eclipse hat einen eingebauten Updatemanager, dieser arbeitet mit so genannten Updatesites als Quelle.<br />");
    _builder_1.newLine();
    _builder_1.append("In Eclipse öffne das <i>\"Help\"</i> Menü und klicke auf <i>\"Install new Software...\"</i>.");
    _builder_1.newLine();
    CharSequence _faqEntry = this.faqEntry("Was ist eine Updatesite?", _builder_1);
    _builder.append(_faqEntry, "\t\t\t\t    ");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t    ");
    StringConcatenation _builder_2 = new StringConcatenation();
    _builder_2.append("<p>Der JEE6 Generator frei verfügbar unter der <a href=\"http://www.eclipse.org/legal/epl-v10.html\">Eclipse Public License</a>.</p>");
    _builder_2.newLine();
    CharSequence _faqEntry_1 = this.faqEntry("Was ist die Lizenz des JEE6 Generators?", _builder_2);
    _builder.append(_faqEntry_1, "\t\t\t\t    ");
    _builder.newLineIfNotEmpty();
    _builder.append("\t\t\t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t  \t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t\t\t");
    _builder.append("<div class=\"span1\">&nbsp;</div>");
    _builder.newLine();
    _builder.append("\t\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("</div>");
    _builder.newLine();
    return _builder;
  }
  
  public CharSequence faqEntry(final String question, final CharSequence answer) {
    StringConcatenation _builder = new StringConcatenation();
    final String key = question.replaceAll("\\W", "_");
    _builder.newLineIfNotEmpty();
    _builder.append("<div class=\"accordion-group\">");
    _builder.newLine();
    _builder.append("  ");
    _builder.append("<div class=\"accordion-heading\">");
    _builder.newLine();
    _builder.append("\t");
    _builder.append("<a class=\"accordion-toggle\" data-toggle=\"collapse\" data-parent=\"#accordion2\" href=\"#");
    _builder.append(key, "\t");
    _builder.append("\"\">");
    _builder.newLineIfNotEmpty();
    _builder.append("\t  ");
    _builder.append(question, "\t  ");
    _builder.newLineIfNotEmpty();
    _builder.append("\t");
    _builder.append("</a>");
    _builder.newLine();
    _builder.append("  ");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("  ");
    _builder.append("<div id=\"");
    _builder.append(key, "  ");
    _builder.append("\" class=\"accordion-body collapse\">");
    _builder.newLineIfNotEmpty();
    _builder.append("\t");
    _builder.append("<div class=\"accordion-inner\">");
    _builder.newLine();
    _builder.append("\t  ");
    _builder.append(answer, "\t  ");
    _builder.newLineIfNotEmpty();
    _builder.append("\t");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("  ");
    _builder.append("</div>");
    _builder.newLine();
    _builder.append("</div>");
    _builder.newLine();
    return _builder;
  }
}
